...
data_nhl |>
  ...
  ggplot(
    mapping = aes(x = points, y = birth_month_cat)
         ) +
  geom_density_ridges() +
...
wflow_lr_best_fit <- fit(wflow_lr_best, data = data_nhl_train)

wflow_dt_all_fit <- fit(wflow_dt_all, data = data_nhl_train)

wflow_dt_best_fit <- fit(wflow_dt_best, data = data_nhl_train)
```
