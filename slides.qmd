...
into a category named "Other". 
    * What message does that send to students who are non-binary or trans? 
:::

. . .

::: {.midi}
- What if we created two levels for racial groups---"White" and "Non-White".
...
from plotnine.geoms import geom_density_2d
...
wflow_lr_all_fit <- fit(wflow_lr_all, data = data_nhl_train)
```
:::
...
pred_lr_all <- predict(wflow_lr_all_fit, new_data = data_nhl_test) |> 
...
final_model <- wflow_dt_all |> fit(data_nhl)
```
